...
void write_pixels(BITMAPFILEHEADER *bfh, BITMAPINFOHEADER *bih, FILE *fp, unsigned char *image, int row_size) {
  for (int i = 0; i < bih->biHeight; i++) {
    // seek to the beginning of the current row
    fseek(fp, bfh->bfOffBits + i * row_size, SEEK_SET);

    // write the current row
    fwrite(&image[i * row_size], row_size, 1, fp);
  }
}
...
void grayscale(BITMAPFILEHEADER *bfh, BITMAPINFOHEADER *bih, unsigned char *image, int row_size, const char* file) {
  ...
  //fwrite(bfh, sizeof(BITMAPFILEHEADER), 1, gray_file);
  //fwrite(bih, sizeof(BITMAPINFOHEADER), 1, gray_file);
  //fwrite(image, (row_size * bih->biHeight), 1, gray_file);
...
int main(int argc, char *argv[]) {
  char *in = argv[1]; // stores the path to bmp file
  char *out = argv[2];
...
// histogram
  ...
  for (int i = 0; i < bih.biHeight; i++) {
    ...
    fread(&image[i * row_size], row_size, 1, fp);

    // skip any padding at the end of the row (it should be useless since we are using fseek from the beginning, but I'm leaving it just in case )
    //fseek(fp, row_size - bih.biWidth * 3, SEEK_CUR);
  }
